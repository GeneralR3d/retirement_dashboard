...
**`app/components/footer.tsx`** — site-wide footer rendered in `app/layout.tsx` below `<main>`. Contains copyright line and links to `/terms`, `/privacy`, and `/about` (the latter two are placeholder routes, not yet built). The layout wraps children in `flex flex-col` so the footer sits flush at the bottom of every page without any page needing to include it explicitly.
