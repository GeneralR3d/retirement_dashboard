...
- **Loan eligibility limits.** `maxLoanLtv = flatPrice * LTV_CAP` (0.75) and `maxLoanMsr = maxLoanForMonthly(grossMonthlyIncomeAtApplication * MSR_LIMIT (0.30), rate, tenure)` — MSR uses gross monthly salary at `incomeAssessmentAge` (`grossMonthlyIncomeAt`, 0 once past `stopWorkingAge`). `incomeAssessmentAge = btoApplicationAge` normally, but `btoCollectionAge` under the **Deferred Income Assessment (DIA) Scheme** (`btoDownpaymentScheme === "deferred"`), where HDB assesses income at key collection — surfaced via an `InfoTooltip` on the Downpayment scheme input and a "(collection age — DIA Scheme)" label on the card. `maxLoan = min(maxLoanLtv, maxLoanMsr)`; `maxMonthlyMortgage` is its instalment. These are surfaced on the BTO page as an informational "Maximum you can borrow" card (with LTV cap / MSR limit / max monthly mortgage sub-cards, plus a green "within your limit" box when MSR headroom exists) and a tooltip on the Monthly mortgage card.
- `nominalLoan = max(0, flatPrice − totalDownpayment)`; `loanAmount = min(nominalLoan, maxLoan)`. Any shortfall (`loanShortfallToCash = nominalLoan − loanAmount`) is **added to `dp2.fromCash`/`dp2.actualPaid`** (paid at collection, so `flatPrice = totalDownpayment + loanAmount` stays exact) and flows into the cash-flow pages automatically via the existing `exp-2 = dp2.fromCash` override. `loanConstrained` triggers an amber warning suggesting a cheaper flat or higher income.
