...
export default function InputsPanel() {
  ...
  // The panel lives in the persistent (planner) layout, so it mounts before
  // localStorage hydration and stays mounted while other pages call setInputs.
  // Adopt upstream changes only when the draft has no unsaved edits.
  const prevInputsRef = useRef(inputs);
  useEffect(() => {
    const prev = prevInputsRef.current;
    prevInputsRef.current = inputs;
    setDraft((d) => (JSON.stringify(d) === JSON.stringify(prev) ? inputs : d));
  }, [inputs]);
            ...
            <tbody>
              {investments.map((inv) => (
                <tr key={inv.id} className="border-b border-foreground/5 hover:bg-foreground/[0.02]">
                  ...
                  {/* Value */}
                  <td className="py-2 px-2">
                    <div className="glass-field flex items-center w-28">
