...
import { useBtoEffectiveExpenses } from "@/app/components/lumpsum-tables";
...
export default function AccumulationPage() {
  ...
  const { effectiveExpenses } = useBtoEffectiveExpenses(inputs, inputs.lumpsumExpenses);
  ...
  // All expenses fed to buildAccumulation: saved inputs (with BTO DP overrides) + mortgage cash
  const allAccumulationExpenses = useMemo(
    () => [...effectiveExpenses, ...workingYearMortgageLumpsums],
    [effectiveExpenses, workingYearMortgageLumpsums],
  );
  ...
  const rows = useMemo(
    () =>
      buildAccumulation({
        ...
        lumpsumInflows: inputs.lumpsumInflows,
        cashStart: cash,
